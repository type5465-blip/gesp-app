...
{id: '2025-12',
        ...
        questions: [
            ...
            { type:'single', q:'下⾯有关 Python 变量的说法，正确的是 ( )。', options:["不可以⽤ for 作为变量名，因为 for 是Python 的关键字（保留字）。", "_tnt 不可以是变量名，因为变量名的第⼀个字符必须是英⽂字母。", "_tnt_ 不可以是变量名，因为最后⼀个字符容易与减号混淆。", "可以⽤ print 作为变量名，因为 print 是关键字，但这不是好习惯，因为 print 有约定的功能与含义。"], answer:0, explain:'本题正确答案为正选项。详细解析请参考CCF GESP官方真题答案解析文档。' },
            ...
            { type:'single', q:'下⾯的 Python 代码执⾏时如果先输⼊ 10 回车后输⼊ 20 并回车，其输出是 ( )。', options:["30=30", "1020=1020", "int(N+M)=1020", "错误提⽰"], answer:2, explain:'本题正确答案为正选项。详细解析请参考CCF GESP官方真题答案解析文档。' },
            { type:'single', q:'某个整数很长很长，形如： 1232123212321…… ，其规律是从 1 开始逐⼀升⾼到 3 然后逐⼀降低到 1 ，然后⼜逐 ⼀升⾼到 3 ，⼀直到很长很长。编写程序，判断从左开始第 N 位数是⼏？假设从左到右第 1 位为 1 。在横线处应该填⼊ 的代码是 ( ) 。', options:["N % 4", "N // 4", "N % 3", "N // 3"], answer:0, explain:'本题正确答案为正选项。详细解析请参考CCF GESP官方真题答案解析文档。' },
            { type:'single', q:'下⾯ Python 代码执⾏后的输出是 ( ) 。', options:["99 99", "100 99", "99 100", "100 100"], answer:1, explain:'本题正确答案为正选项。详细解析请参考CCF GESP官方真题答案解析文档。' },
            { type:'single', q:'有关下⾯ Python 代码的说法，错误的是 ( ) 。a = 3 b = a = 4 print(a, b)1 2 3 N = input("第一个数 :") M = input("第二个数 :") print(f"int(N+M)={int(N+M)}")1 2 3 N = int(input("请输入编号： ")) M = ________________ if M != 0: print(', options:["range(1,10,2) 改为 range(1,11,2) 结果相同。", "range(1,10,2) 改为 range(0,10,2) 结果相同。", "tnt += i 与 tnt = tnt + i 效果相同。", "tnt += i 与 tnt = i + tnt 效果相同。"], answer:1, explain:'本题正确答案为正选项。详细解析请参考CCF GESP官方真题答案解析文档。' },
            { type:'single', q:'下⾯ Python 代码执⾏后输出是 ( ) 。', options:["10#20#30#40#50#60#70#80#90#", "90#", "90", "没有输出"], answer:1, explain:'本题正确答案为正选项。详细解析请参考CCF GESP官方真题答案解析文档。' },
            { type:'single', q:'两个正整数，只要不相等，就⼀直进⾏如下操作：最⼤数减去最⼩数得到⼀个值，该值和两个数的最⼩数 构成新的两个正整数，重复操作，直到两个数相等，此时输出该数。下⾯的 Python 代码⽤于实现该操作，横线处应 该填写的代码是 ( ) 。', options:["N = N - M", "M = M - N", "P = N - M", "N = M"], answer:0, explain:'本题正确答案为B选项。详细解析请参考CCF GESP官方真题答案解析文档。' },
            { type:'single', q:'如果⼀个正整数能被 3 整除，或者某⼀位能被 3 整除，则称之为 “ 漂亮数 ” ，如 12 是漂亮数因为能被 3 整除， 311是漂亮数因为含有 3 ， 22 不是漂亮数因为不能被 3 整除也不含有 3 。下⾯的 Python ⽤于判断正整数 N 是否为漂亮 数，横线处应该填⼊的代码是 ( ) 。', options:["N % 10 == 0", "N % 3 % 10 == 0", "N % 10 % 3", "N % 10 % 3 == 0"], answer:3, explain:'本题正确答案为正选项。详细解析请参考CCF GESP官方真题答案解析文档。' },
            { type:'single', q:'下⾯的 Python 代码执⾏后海⻳最终朝向是 ( ) 。（默认初始朝向东，即屏幕右侧）', options:["东（ 0°）", "南（ 270° 或 -90° ）", "西（ 180° ）", "北（ 90°）"], answer:1, explain:'本题正确答案为正选项。详细解析请参考CCF GESP官方真题答案解析文档。' },
            { type:'single', q:'为在 Python Turtle 中输出如下图形，代码横线处应填⼊ ( ) 。', options:["turtle.home()", "turtle.goto(0,0)", "turtle.reset()N = int(input()) Flag = \"非漂亮数 \" if N % 3 == 0: Flag = \"漂亮数 \" else: while N != 0: if ____________________: Flag = \"漂亮数 \" break N //= 10 print(Flag)1 2 3 4 5 6 7 8 9 1 0 1 1 1 2 import tur", "turtle.home(0,0)"], answer:1, explain:'本题正确答案为正选项。详细解析请参考CCF GESP官方真题答案解析文档。' },
            { type:'single', q:'为在 Python Turtle 中输出如下图形，代码横线处应填⼊ ( ) 。', options:["20 * i", "20 * (i + 1)", "20 * i, steps = 4", "20 * (i + 1), steps = 4 2"], answer:3, explain:'本题正确答案为正选项。详细解析请参考CCF GESP官方真题答案解析文档。' },
            // Q16-25
